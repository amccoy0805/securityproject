...
class AegisClient:
    ...
    def messages(self, model: str, messages: list[dict[str, Any]], **kwargs: Any) -> dict[str, Any]:
        body = {"model": model, "messages": messages, **kwargs}
        return self._post("/v1/messages", body)

    def check(self, text: str, *, model: str | None = None, scan_response: bool = False) -> dict[str, Any]:
        return self._post(
            "/v1/policy/check",
            {"text": text, "model": model, "scan_response": scan_response},
        )

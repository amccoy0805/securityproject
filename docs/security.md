...
| **Overbroad permissions / unintended actions**: the model decides to call `delete_account` or `pay_vendor` on its own. | Tool registry + action classifier (`destructive`, `financial`, `write`, …). Sensitive calls are stripped from the response unless the request carries `X-Aegis-Approve-Action: 1`. Financial calls also enforce a USD threshold. |
| **Supply-chain / plugin risk**: a third-party plugin teaches the model a new tool, or mutates an existing tool's argument schema. | Every tool the customer wants advertised must be registered in advance. Aegis stores a SHA-256 of the schema; advertising a mutated schema returns HTTP 451 `tool_registry_block`. |
| **SSRF via tool arguments**: an injected page tells the agent's `http_request` tool to hit `http://169.254.169.254/`. | URL safety on tool arguments blocks loopback, link-local, RFC1918, cloud metadata, and disallowed schemes by default. Per-tool allow/deny lists for finer control. |
| **Credential theft from `.env`**: attacker copies an `aeg_…` key and uses it from elsewhere. | Per-key CIDR allowlist (no override). Optional first-seen-IP pin to /24 or /48 with audited override. `first_seen_ip` / `last_used_ip` recorded on every key for forensics. |
